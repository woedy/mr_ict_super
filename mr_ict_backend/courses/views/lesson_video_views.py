...
from courses.models import Lesson, LessonVideo
from courses.views.serializers import AllLessonVideosSerializer, LessonVideoDetailsSerializer
...
def add_lesson_video_view(request):
    payload = {}
    data = {}
    errors = {}
    ...
    if request.method == 'POST':
        ...
        new_lesson_video = LessonVideo.objects.create(
            lesson=lesson,
...
@permission_classes([IsAuthenticated])
@authentication_classes([TokenAuthentication])
def get_all_lesson_videos_view(request):
    payload = {}
    data = {}
    errors = {}
    ...
    all_lesson_videos = LessonVideo.objects.filter(is_archived=False)
    ...
    all_lesson_videos_serializer = AllLessonVideosSerializer(paginated_lesson_videos, many=True)

    # Prepare the response data
    data['videos'] = all_lesson_videos_serializer.data
    data['pagination'] = {
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication, ])
def get_lesson_video_details_view(request):
    payload = {}
    data = {}
    errors = {}
    ...
    if not id:
        errors['id'] = ["Lesson  Video id required"]

    try:
        lesson_video = LessonVideo.objects.get(id=id)
    except:
        errors['id'] = ['Lesson  Video does not exist.']
    ...
    lesson_video_serializer = LessonVideoDetailsSerializer(lesson_video, many=False)
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication, ])
def archive_lesson_video(request):
    payload = {}
    data = {}
    errors = {}
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication, ])
def unarchive_lesson_video(request):
    payload = {}
    data = {}
    errors = {}
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication, ])
def get_all_archived_lesson_video_view(request):
    payload = {}
    data = {}
    errors = {}
    ...
    all_lesson_videos = LessonVideo.objects.all().filter(is_archived=True)
...
@permission_classes([IsAuthenticated, ])
@authentication_classes([TokenAuthentication, ])
def delete_lesson_video(request):
    payload = {}
    data = {}
    errors = {}
    ...
    if request.method == 'POST':
        ...
        if not id:
            errors['id'] = ['Lesson  Video ID is required.']
