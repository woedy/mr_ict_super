...
const LmmTutor = () => {
  ...
  useEffect(() => {
    fetch(`${baseUrl}llm-tutor/api/lesson/semantic_html_101`)
      .then((res) => {
        if (!res.ok) {
  ...
  const handleCodeChange = async (newCode) => {
    ...
    try {
      const response = await fetch(`${baseUrl}llm-tutor/api/llm/update`, {
        method: 'POST',
        headers: {
  ...
  const handleAskQuestion = async () => {
    ...
    setAssignmentFeedback('');
    try {
      const response = await fetch(`${baseUrl}llm-tutor/api/llm/ask`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
