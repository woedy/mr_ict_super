...
const ArchivedSchools = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/schools/get-all-archived-schools/?search=${encodeURIComponent(
          search,
        )}&page=${page}`,
        {
          headers: {
  ...
  const handleArchive = async (itemId) => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/schools/unarchive-school/`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div>
      <Breadcrumb pageName="Archived Schools" />
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="col-span-2 rounded-sm border border-stroke  shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          {schools
            ? schools.map((school) => (
                <div
                  className="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-6 md:px-6 2xl:px-7.5 hover:bg-graydark"
                  key={school.school_id}
                >
                  <div className="col-span-2 flex items-center">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div className="h-12 w-12 overflow-hidden rounded-md mr-2">
                          <img
                            src={`${baseUrlMedia}${school.photo}`}
                            alt="school photo"
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div className="h-12 w-12 overflow-hidden rounded-md mr-2">
                          <img
                            src={`${baseUrlMedia}${school.school_logo}`}
                            alt="school logo"
                            className="h-full w-full object-cover"
                          />
                        </div>
                  ...
                  <div className="col-span-1 hidden items-center sm:flex">
                    <p className="text-sm text-black dark:text-white">
                      <div className="flex items-center space-x-3.5">
                        ...
                        <button
                          onClick={() => openArchiveModal(school.school_id)}
                        ...
                        <button
                          className="hover:text-primary"
                          onClick={() => openDeleteModal(school.school_id)} // Pass the ID of the item to be deleted
                        >
                          <svg
                            className="fill-current"
                            width="18"
